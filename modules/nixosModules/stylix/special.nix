{ inputs, lib, ... }:
{
  ...
  flake.nixosModules.core =
    ...
    in
    {
      ...
      # };
      # };
      specialisation = lib.lists.foldr (a: b: a // b) { } (
        lib.lists.forEach
          [
            {
              name = "gruvbox";
          ...
          (c: {
            ${c.name}.configuration = {
              config.stylix = {
                base16Scheme = lib.lists.foldr (a: b: a // b) { } (
                  lib.lists.forEach bases (b: {
                    ${b} = c.${b};
                  })
                );
                image =
                  let
                    themeFile = builtins.toFile "${c.name}-theme.json" (
                      builtins.toJSON {
                        name = c.name;
                  ...
                  in
                  lib.mkForce (
                    pkgs.runCommand "${c.name}-image.png" { HOME = "$TMPDIR"; } ''
